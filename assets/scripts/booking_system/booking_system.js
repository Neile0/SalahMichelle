...
var testStyD = getStylistByDate(5,"TUE");
...
function selectDate(selected){
    ...
    if (stylists.length > 0){
        for (var i = 0; i < stylists.length; i ++){
            ...
            var headerText = document.createTextNode("Appointments with " + stylistName + " on " + selectedDate);
...
function selectTime(selected){
    ...
    var eDetails = document.getElementById("appointment-details");
    var node = document.createElement("p")
    var text = document.createTextNode("You have chosen an appointment for a " + selectedService.type.toLowerCase() + " " + selectedService.title + " on " + selectedDate + " at " + convertAMPM(selectedStart) + " with " + selectedStylist.firstName);

    node.appendChild(text);
    eDetails.appendChild(node);
    var btn = document.createElement("button")
    var btnText = document.createTextNode("CONFIRM");
    btn.setAttribute("id","btn-appointment-details-confirm")
    btn.setAttribute("onclick","confirmAppointmentDetails()")
    btn.appendChild(btnText);
    eDetails.appendChild(btn);
 
}

function confirmAppointmentDetails(){
    alert("click")
    var eConfirmBtn = document.getElementById("btn-appointment-details-confirm")
    eConfirmBtn.style.display = "none";

    var eUserDetails = document.getElementById("user-details-block")
    eUserDetails.style.display = "block";
}
...
function getServicesByType(type){
    ...
    getRequest("/SalahMichelle/assets/scripts/booking_system/get_services_by_type.php?type="+type,getFunction);
    return ls;
}
...
function getStylistProvides(serviceId){
    ...
    getRequest("/SalahMichelle/assets/scripts/booking_system/get_stylist_provides.php?service="+serviceId,getFunction);
    return ls;
}
...
function getWorkingHours(staffId){
    ...
    getRequest("/SalahMichelle/assets/scripts/booking_system/get_working_hours.php?id="+staffId,getFunction);
    return ls;
}
...
function getStylistByDate(serviceId,day){
    ...
    getRequest("/SalahMichelle/assets/scripts/booking_system/get_stylist_by_date.php?id="+serviceId+"&day="+day,getFunction);
    return ls;
}
...
function getAppointmentTimes(staffId,date,start,end,duration){
    ...
    getRequest("/SalahMichelle/assets/scripts/booking_system/get_appointment_times.php?staffId="+staffId+"&date="+date+"&start="+start+"&end="+end+"&duration="+duration,getFunction);
    return ls;
}
